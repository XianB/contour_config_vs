...
int pack_template(const Koyo_Contour_Template_Runtime_Param &koyo_contour_template_runtime_param, char *buf, int buf_free, int *bufsize)
{
...
static int do_create_template(const cv::Mat &src, const cv::Mat &bitMap, Koyo_Tool_Contour_Parameter* koyo_tool_contour_parameter, Koyo_Contour_Template_Runtime_Param &koyo_contour_template_runtime_param)
{
 ...
 //   tt1.start();
    ...
    if (koyo_tool_contour_parameter->sensitivity == CONTOUR_ACCURACY_LOW) {
        ...
        sensitity_threshold_low = CANNY_ACCMEDIUM_THRLOW;
        sensitity_threshold_high = CANNY_ACCMEDIUM_THRHIGH;
...
//    tt.start();
    ...
    std::cout << "angle_range is : " <<  koyo_tool_contour_parameter->angle_range << std::endl;
    for (int i = 0; i < optimal_pyr_level; ++i) {
        ...
        for (double j = -(koyo_tool_contour_parameter->angle_range); (int)j < koyo_tool_contour_parameter->angle_range; j += angle_steps[i]) {
...
//    print_tpls(std::cout, koyo_contour_template_runtime_param);
    ...
    for (int i = 0; i < kctrp.run_time_npyramid; ++i) {
        ...
        for (double j = -45; (int)j < 45; j += angle_steps_debug[i]) {
 ...
 *  提供给客户端的接口
 * */
int create_template(const UINT8 *yuv, Koyo_Tool_Contour_Parameter *koyo_tool_contour_parameter, char *buf, int buf_free, int *buf_size)
{

//    QString filename1 = "C:\\Users\\1503\\Desktop\\transferBitmapcreate_templateNew.txt";
...
#ifdef  DEBUG_PRINT
    //print_debug_info(pyramid_templates_debug, buf);
#endif
...
int get_contours(const UINT8 *yuv, UINT8 *contours[3])
{
    ...
    cv::Canny(src, contour_high, CANNY_ACCHIGH_THRLOW, CANNY_ACCHIGH_THRHIGH);
    for (int i = 0; i < HEIGHT; ++i) {
        for (int j = 0; j < WIDTH; ++j) {
